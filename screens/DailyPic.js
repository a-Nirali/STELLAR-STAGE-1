...
export default class DailyPicScreen extends Component {
    ...
    renderVideo = () => {
        <TouchableOpacity style={styles.listContainer}
            onPress={() => Linking.openURL(this.state.DailyPic.url).catch(err => console.error("Couldn't load page", err))}
        >
    ...
    render() {
        const url = this.state.DailyPic.url
        if (Object.keys(this.state.DailyPic).length === 0) {
            return (
                <View
                    ...
                    <ImageBackground source={require('../assets/stars.gif')} style={styles.backgroundImage}>
                        ...
                        <ScrollView style={styles.listContainer}>
                            ...
                            <View style={{ padding: 20 }}>
                                <Text style={styles.titleText}>{this.state.DailyPic.title}</Text>
